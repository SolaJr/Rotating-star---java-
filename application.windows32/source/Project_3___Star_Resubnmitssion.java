...
public class Project_3___Star_Resubnmitssion extends PApplet {
...
public void star(float x, float y, float r1, float r2, int npoints) {
  ...
  for (float angle = 0; angle < TWO_PI; angle += theta) {
    ...
    delta_x = x + cos(angle + halftheta ) * r1;
    delta_y = y + sin(angle + halftheta ) * r1;
    vertex(delta_x, delta_y);
  }
  endShape(CLOSE);
}
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "Project_3___Star_Resubnmitssion" };
    if (passedArgs != null) {
